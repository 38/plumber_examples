...
static int _init(uint32_t argc, char const* const* argv, void* ctxbuf)
{
	...
	/* The only change we have made is the servlet should tell framework
	 * Do not release the connection after the event has been processed.
	 * Instead, hold the TCP connection until the next input becomes availible */
